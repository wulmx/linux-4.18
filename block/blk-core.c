...
blk_qc_t generic_make_request(struct bio *bio)
{
	...
	if (!generic_make_request_checks(bio))//wlm:判断bio是否合法
		goto out;

	/* 因为 make_request_fn 只能触发一次，因此通过判断bio_list中是否有bio
	 * 如果有则把bio加入到尾部等待处理，如果没有则初始化bio_list并处理bio。
	...
	do {
		struct request_queue *q = bio->bi_disk->queue;//获得bio对应的设备队列
		...
		if (likely(bio_queue_enter(bio) == 0)) {
			...
			bio_list_init(&bio_list_on_stack[0]);
			ret = do_make_request(bio);//将bio进行进一步处理，放入块设备层的处理队列中
