 ...
 */
struct blk_mq_tags {
	unsigned int nr_tags;//wlm:一般情况，tag数量等于最大队列深度(QD)
	unsigned int nr_reserved_tags;//一般情况，保留tag数量为0
	/* 活跃队列数量,blk-mq中一个tag set一是多个request queue共享
	 * 记录当前活跃队列数量目的是为均匀分配tag到每个request queue
	 */
