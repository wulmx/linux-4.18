	...
	const char *name)
{
	...
	vring_init(&vring, num, queue, vring_align);//wlm:初始化vring内存布局
	//wlm: 构建vring_virtqueue结构体
...
/* Only available for split ring 虽然创建vring_virtqueue，但返回的是virtqueue，使用是需要转换*/
					...
					const char *name)
{
	...
	/* Put everything in free lists. */
	vq->free_head = 0;
	for (i = 0; i < vring.num-1; i++)//wlm:串联所有的desc
...
ssize_t get_vqs_data(struct device *_d, char *buf)
{
...
EXPORT_SYMBOL_GPL(get_vqs_data);
