 ...
 * request 是bio 提交给IO调度器产生的数据，一个request 
 * 中放着顺序排列的bio,当设备提交 bio 给IO调度器时，
 * IO调度器可能会插入bio，或者生成新的request 
 */
struct request {
	struct request_queue *q;//代表一个物理设备，顺序存放request
	...
	/* the following two fi elds are internal, NEVER access directly */
	...
	u64 start_time_ns;//wlm:request生成的时间戳
	/* Time that I/O was submitted to the device. */
	u64 io_start_time_ns;//wlm:提交给驱动层的开始时间
	 ...
	 * completion callback.
	 */
	rq_end_io_fn *end_io;//wlm:回调函数 flush_end_io
	void *end_io_data;
...
struct request_queue {
	...
	/* 在 blk_mq_init_allocated_queue 中赋值 */
